...
test.describe('Language Switching', () => {
  ...
  test('should switch from English to French', async ({ page }) => {
    ...
    const frenchLink = page
      .locator('a[href*="/fr"], button:has-text("FR"), button:has-text("Français")')
      .first();
  ...
  test('should preserve page path when switching language', async ({ page }) => {
    ...
    if (await englishLink.isVisible()) {
      ...
      // Should be on English products page with localized slug
      expect(page.url()).toContain('/en/products');
    }
  });

  test('should load localized English products URL', async ({ page }) => {
    const response = await page.goto('/en/products');
    expect(response?.status()).toBe(200);
    expect(page.url()).toContain('/en/products');
  });

  test('should redirect legacy French slug on EN locale to localized path', async ({ request }) => {
    const response = await request.get('/en/produits', { maxRedirects: 0 });
    expect(response.status()).toBe(301);
    expect(response.headers().location).toMatch(/\/en\/products\/?$/);
  });
...
test.describe('Language Switching - Accessibility', () => {
  test('should have accessible language switcher', async ({ page }) => {
    ...
    const languageSwitcher = page
      .locator('[aria-label*="langue"], [aria-label*="language"]')
      .first();
