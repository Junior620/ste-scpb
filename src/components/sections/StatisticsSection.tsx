    ...
    <div className="space-y-12">
      ...
      <section className="bg-background-secondary rounded-xl p-4 md:p-6 border border-border">
        <div className="flex flex-wrap items-center gap-4">
          ...
          >
            {PERIODS.map(period => (
              <option key={period} value={period}>{t(`filters.periods.${period}`)}</option>
            ))}
          </select>
          ...
          >
            {REGIONS.map(region => (
              <option key={region} value={region}>{t(`filters.regions.${region}`)}</option>
            ))}
          </select>

          <div className="flex items-center gap-2 ml-auto">
            <button onClick={shareStats} className="p-2 text-foreground-muted hover:text-foreground" title="Partager">
              <Share2 className="w-4 h-4" />
            </button>
            <button onClick={resetFilters} className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-foreground-muted hover:text-foreground">
        ...
        <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-border text-xs text-foreground-muted">
          ...
          {t('filters.lastUpdated')}: {new Date(data.lastUpdated).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}
        </div>
      </section>
      ...
      {!hasNoData && (
        <ScrollReveal direction="up" delay={100} duration={500}>
          ...
          <div className="bg-background-secondary rounded-xl p-6 border border-border">
            <h3 className="text-lg font-semibold text-foreground mb-4">{t('destinations.top')}</h3>
            <div className="space-y-3">
              {(filteredDestinations.length > 0 ? filteredDestinations : data.topDestinations).slice(0, 5).map((dest, idx) => (
                <div key={dest.countryCode} className="flex items-center gap-3 group">
                  ...
                  <div className="flex-1">
                    ...
                    {dest.port && <p className="text-xs text-foreground-muted opacity-0 group-hover:opacity-100 transition-opacity">Port: {dest.port}</p>}
                  </div>
                  <span className="text-sm font-semibold text-primary">{dest.percentage}%</span>
                </div>
              ))}
            </div>
            <Link href="/a-propos#presence" className="flex items-center gap-1 text-sm text-primary hover:underline mt-4">
        ...
        <ScrollReveal direction="up" delay={150} duration={500}>
          ...
          <div className="bg-background-secondary rounded-xl p-6 border border-border">
            ...
            <p className="text-[10px] text-foreground/40 mb-5">
              Part du volume exporté (12 derniers mois) — Total : {data.kpi.tonnesExported.toLocaleString()} t
            </p>
            ...
            <div className="relative">
              ...
              <div className="absolute top-0 left-1/4 -translate-x-1/2 text-[8px] text-foreground/25">25%</div>
              <div className="absolute top-0 left-1/2 -translate-x-1/2 text-[8px] text-foreground/25">50%</div>
              <div className="absolute top-0 left-3/4 -translate-x-1/2 text-[8px] text-foreground/25">75%</div>
              
              <div className="space-y-3 relative">
                {(productFilter === 'all' ? data.productMix : filteredProductMix).map((product, idx) => {
                  ...
                  const autresTooltip = isAutres ? 'Hévéa 20t, Maïs 15t, Bois 10t, +2 autres' : null;
                  
                  return (
                    <button 
                      key={product.slug} 
                      onClick={() => setProductFilter(isSelected ? 'all' : product.slug as ProductFilter)} 
                      className={`w-full text-left group relative rounded-lg p-2 -mx-2 transition-colors ${
                        isSelected ? 'bg-primary/10 ring-1 ring-primary/30' : 'hover:bg-foreground/5'
                      }`}
                    >
                      <div className="flex items-center justify-between text-sm mb-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold tabular-nums ${
                            idx === 0 ? 'bg-primary/25 text-primary' : 
                            idx < 3 ? 'bg-foreground/10 text-foreground/60' : 
                        ...
                        <div className="flex items-center gap-3 tabular-nums">
                          <span className="text-[10px] text-foreground/35 min-w-[45px] text-right">{product.volume.toLocaleString()} t</span>
                          <span className="text-foreground font-semibold min-w-[32px] text-right">{product.percentage}%</span>
                        </div>
                      </div>
                      ...
                      <div className="absolute left-1/2 -translate-x-1/2 -top-14 px-3 py-2 bg-background/95 backdrop-blur-sm border border-border rounded-lg shadow-xl text-xs whitespace-nowrap z-20 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-foreground/30">•</span>
                          <span className="text-foreground/50">{product.volume.toLocaleString()} t</span>
                        </div>
                        {autresTooltip && (
      ...
      <ScrollReveal direction="up" delay={0} duration={500}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <QualityCard icon={<BarChart3 className="w-6 h-6" />} title={t('quality.grading.title')} description={t('quality.grading.description')} />
          <QualityCard icon={<Shield className="w-6 h-6" />} title={t('quality.traceability.title')} description={t('quality.traceability.description')} />
          <QualityCard icon={<Leaf className="w-6 h-6" />} title={t('quality.phyto.title')} description={t('quality.phyto.description')} />
          <QualityCard icon={<FileCheck className="w-6 h-6" />} title={t('quality.certifications.title')} description={t('quality.certifications.description')} />
        </div>
        <div className="flex flex-wrap gap-3 mt-6 pt-6 border-t border-border">
          <Button variant="outline" size="sm"><Download className="w-4 h-4 mr-2" />{t('quality.downloadCOA')}</Button>
          <Button variant="outline" size="sm"><FileCheck className="w-4 h-4 mr-2" />{t('quality.downloadOrigin')}</Button>
      ...
      {productData && (
        <section className="bg-primary/5 rounded-xl p-6 md:p-8 border border-primary/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {productData.formats.map(format => (
                  <span key={format} className="px-3 py-1.5 bg-background rounded-full text-sm text-foreground border border-border">{format}</span>
                ))}
              </div>
            </div>
            <div>
            ...
            <div>
              <h3 className="font-medium text-foreground mb-3">{t('productFocus.certifications')}</h3>
        ...
        <div className="space-y-2">
          {['volumes', 'update', 'products', 'methodology'].map((key) => (
            <div key={key} className="border border-border rounded-lg overflow-hidden">
              <button type="button" onClick={() => toggleFaq(key)} className="w-full flex items-center justify-between p-4 text-left hover:bg-background transition-colors">
                <span className="font-medium text-foreground">{t(`faq.${key}.question`)}</span>
                {expandedFaq === key ? <ChevronUp className="w-5 h-5 text-foreground-muted" /> : <ChevronDown className="w-5 h-5 text-foreground-muted" />}
              </button>
              {expandedFaq === key && <div className="px-4 pb-4 text-sm text-foreground-muted">{t(`faq.${key}.answer`)}</div>}
            </div>
          ))}
        </div>
      </section>
      ...
      <ScrollReveal direction="up" delay={0} duration={500}>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <Button asChild size="lg"><Link href="/devis"><Ship className="w-5 h-5 mr-2" />{t('cta.quote')}</Link></Button>
          <Button asChild variant="outline" size="lg"><Link href="/devis?type=sample"><Package className="w-5 h-5 mr-2" />{t('cta.sample')}</Link></Button>
          <Button asChild variant="ghost" size="lg"><Link href="/contact"><MessageSquare className="w-5 h-5 mr-2" />{t('cta.contact')}</Link></Button>
        </div>
...
function KPICard({ icon, value, label, tooltip }: { icon: React.ReactNode; value: string; label: string; tooltip?: string }) {
  return (
...
function VolumeChart({ data, lastUpdated }: VolumeChartProps) {
  ...
  const monthLabels: Record<string, string> = {
    ...
    'Mai': 'Mai', 'Juin': 'Juin', 'Juil': 'Juil', 'Août': 'Août',
    'Sep': 'Sep', 'Oct': 'Oct', 'Nov': 'Nov', 'Déc': 'Déc'
  };
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="relative" style={{ height: chartHeight + 40 }}>
        ...
        <div className="absolute left-0 top-0 bottom-6 w-9 flex flex-col justify-between text-[10px] text-foreground/50 tabular-nums">
          {[...gridLines].reverse().map(v => (
            <span key={v} className="text-right pr-1">{v}t</span>
          ))}
        </div>
        ...
        {/* Bars container */}
        <div className="absolute left-10 right-0 bottom-6 flex items-end gap-1" style={{ height: chartHeight }}>
          {data.map((item, idx) => {
            ...
            return (
              <div key={idx} className="flex-1 flex flex-col items-center group relative">
                ...
                >
                  ...
                  <div 
                    ...
                    style={{ 
                      ...
                      transform: show3D ? 'translateX(5px) translateY(4px)' : 'translateX(3px) translateY(2px)',
                  ...
                  >
                    ...
                    <div className={`absolute inset-x-0 top-0 rounded-t ${show3D ? 'h-[3px] bg-gradient-to-r from-white/40 via-white/55 to-white/30' : 'h-[2px] bg-gradient-to-r from-white/25 via-white/35 to-white/20'}`} />
                    {/* Inner glow */}
                    <div className={`absolute inset-x-0 top-0 rounded-t bg-gradient-to-b from-white/15 to-transparent ${show3D ? 'h-1/3' : 'h-1/4'}`} />
                  </div>
                  
                  {/* Tooltip */}
                  <div className={`absolute left-1/2 -translate-x-1/2 px-3 py-2 bg-background/95 backdrop-blur-sm border border-border rounded-lg shadow-xl text-xs whitespace-nowrap z-30 transition-all duration-150 pointer-events-none ${
                    isSelected ? 'opacity-100' : 'opacity-0 group-hover/bar:opacity-100'
                  }`} style={{ bottom: barHeightPx + (isPeak || isLow ? 32 : 8) }}>
                    <div className="font-medium text-foreground">{item.month} {item.year}</div>
                    <div className="text-primary font-bold text-sm">{item.volume} t</div>
                    {change && (
                      <div className={`text-[10px] mt-0.5 ${change.delta >= 0 ? 'text-green-500' : 'text-red-400'}`}>
                        {change.delta >= 0 ? '+' : ''}{change.delta}t ({change.delta >= 0 ? '+' : ''}{change.percent}%) vs {change.prevMonth}
                      </div>
                    )}
                  </div>
                </button>
                ...
                <span className={`text-[10px] mt-1.5 transition-colors ${
                  isPeak ? 'text-primary font-bold border-b-2 border-primary pb-0.5' : 
                  isLow ? 'text-amber-600 dark:text-amber-400 font-bold border-b-2 border-amber-500/60 pb-0.5' : 
      ...
      {selectedMonth !== null && (
        <div className="mt-3 p-3 bg-primary/5 border border-primary/20 rounded-lg text-sm">
          ...
          <p className="text-xs text-foreground-muted mt-1">
            {selectedMonth === peakIdx && "Période de forte récolte – Disponibilité élevée pour export."}
            {selectedMonth === lowIdx && "Période creuse – Stocks limités, planifiez vos commandes."}
            {selectedMonth !== peakIdx && selectedMonth !== lowIdx && "Disponibilité standard selon collecte et séchage."}
          </p>
        </div>
      )}
      ...
      <div className="mt-3 pt-2 border-t border-border/30 flex items-center justify-between text-[9px] text-foreground/50">
        ...
        <span>Mise à jour : {new Date(lastUpdated).toLocaleDateString('fr-FR', { month: 'short', year: 'numeric' })}</span>
      </div>
    </div>
  );
}
